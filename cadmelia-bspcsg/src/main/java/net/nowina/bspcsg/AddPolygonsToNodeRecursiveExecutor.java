...
public class AddPolygonsToNodeRecursiveExecutor extends AddPolygonsToNodeExecutor {

    public AddPolygonsToNodeRecursiveExecutor(Factory factory) {
        super(factory);
    }
    ...
    public void execute(PolygonList polygonsToAdd, Node node) {
        ...
        for (PolygonListBrowser browser = polygonsToAdd.browse(); browser.hasNext(); browser.next()) {
            Polygon polygon = browser.get();
            addPolygonToNode(polygon, node);
        }
    ...
    protected void addBackPolygon(Polygon polygon, Node node) {
        ...
        addPolygonToNode(polygon, node.getBack());
    }
